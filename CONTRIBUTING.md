...
1. Add the model to `MODELS` dict in `scripts/gen_oracle.py` and `scripts/parity_check.py`
2. Run `uv run python scripts/gen_oracle.py --model <slug>`
3. Add import/export lines to `Makefile` convert target
4. Update `CLAIMS.md` with the new sample sizes
5. Update `README.md` parity matrix
...
4. Parity checker automatically picks up new prompts from oracle directory
